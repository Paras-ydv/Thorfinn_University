            ...
            {PROGRAMS.map((prog) => (
              <div key={prog.level} className="card overflow-hidden">
                <div className="px-5 py-4 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
                  <h2 className="font-semibold text-slate-900 text-sm">{prog.level}</h2>
                ...
                <ul className="divide-y divide-slate-100">
                  {prog.list.map((p) => (
                    <li key={p} className="flex items-center justify-between px-5 py-3 hover:bg-slate-50 transition-colors">
                      <span className="text-sm text-slate-700">{p}</span>
                      <Link href="/departments" className="text-xs text-[#1e3a8a] hover:underline flex items-center gap-1">
          ...
          <div className="space-y-6">
            <div className="card overflow-hidden">
              <div className="px-5 py-4 bg-slate-50 border-b border-slate-200">
                <h3 className="text-sm font-semibold text-slate-700">Academic Calendar 2024–25</h3>
              </div>
              <div className="divide-y divide-slate-100">
                {CALENDAR.map((c) => (
                  <div key={c.event} className="px-5 py-3">
                    <p className="text-xs font-medium text-slate-800">{c.event}</p>
            ...
            <div className="card p-5">
              <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-500 mb-4">Grading System</h3>
