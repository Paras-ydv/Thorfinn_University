...
export function CareerSimulator() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-6">
        <h2 className="text-xl font-bold text-white">Career Path Simulator</h2>
        <p className="text-sm text-gray-400 mt-1">Simulate your journey from interest to placement</p>
      </div>
        ...
        {FLOW.map((step, i) => {
          ...
          return (
            <div key={step.id} className="flex items-center gap-1 flex-shrink-0">
              <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-medium transition-all ${done ? "bg-green-500/20 text-green-400" : active ? "bg-blue-600 text-white" : "glass text-gray-500"}`}>
        ...
        {currentStep < FLOW.length ? (
          <motion.div key={currentStep} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="glass rounded-3xl p-8">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {getOptions(FLOW[currentStep]).map((opt) => (
                <button key={opt} onClick={() => select(opt)} className="glass rounded-xl p-4 text-left text-gray-300 hover:text-white hover:bg-white/10 hover:border-blue-500/50 border border-transparent transition-all text-sm font-medium">
                  {opt}
                </button>
              ))}
            </div>
          ...
          <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <div className="glass rounded-2xl p-6 text-center bg-gradient-to-br from-green-500/10 to-blue-500/10">
              <h3 className="text-xl font-bold text-white mb-2">Your Career Path 🎯</h3>
              <p className="text-gray-400 text-sm">Based on your interest in <span className="text-blue-400 font-semibold">{selections.interest}</span></p>
            </div>
            {FLOW.slice(1).map((step, i) => {
              ...
              return (
                <motion.div key={step.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="glass rounded-2xl p-5 flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-1">{step.label}</p>
                    <p className="text-white font-medium text-sm">{selections[step.id]}</p>
                  </div>
                </motion.div>
              );
            })}
