...
export function HappeningNow() {
  ...
  const fetchEvents = async () => {
    ...
    if (error || !data) return; // silently fall back to demo data
    setDbAvailable(true);
  ...
  const postEvent = async () => {
    ...
    if (dbAvailable) {
      ...
      const { data } = await supabase
        .from("events")
        .insert([{ ...form, votes: 0, user_id: user?.id, expires_at }])
        .select();
      if (data?.[0]) {
  ...
  const upvote = async (id: string) => {
    ...
    if (dbAvailable) {
      ...
      if (current) {
        await supabase.from("events").update({ votes: current.votes + 1 }).eq("id", id);
      }
    }
  };
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Real-time campus events feed
            {!dbAvailable && <span className="ml-2 text-xs text-yellow-500/80">(demo mode)</span>}
          </p>
        </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {TAGS.map((tag) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${form.tag === tag ? "bg-blue-600 text-white" : "glass text-gray-400 hover:text-white"}`}
                >
                  {tag}
                </button>
              ))}
        ...
        {events.map((event, i) => (
          ...
          >
            <button
              ...
              className={`flex flex-col items-center gap-1 min-w-[48px] py-2 px-3 rounded-xl transition-all ${
                votedIds.has(event.id) ? "bg-blue-600 text-white" : "glass text-gray-400 hover:text-blue-400"
              }`}
            >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${TAG_COLORS[event.tag] || "bg-gray-500/20 text-gray-400"}`}>
                  {event.tag}
                </span>
              </div>
            </div>
